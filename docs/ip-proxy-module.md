...
- `换IP轮次`：仅在 `711proxy + API 模式 + 自动跑批成功轮次` 生效，默认 `1`，范围 `1-500`。命中后优先切换当前代理池的下一条 IP。
- `换代理池轮次`：仅在 `711proxy + API 模式 + 自动跑批成功轮次` 生效，默认 `20`，范围 `1-500`。命中后主动从 711 API 拉取新代理池，并启用新池第一条。
- `池尾允许拉新池`：仅影响 `换IP轮次` 命中后的池空/池尾场景。开启时会从 711 API 拉新池并启用新池第一条；关闭时跳过本轮换 IP。`换代理池轮次` 命中时始终主动拉新池，不受该开关限制。

自动跑批轮换顺序：

1. 只统计当前自动跑批 session 内的累计成功轮数。
2. 同一轮同时命中 `换代理池轮次` 和 `换IP轮次` 时，先执行 `换代理池轮次`，并启用新池第一条，不再额外执行一次“下一条”，避免跳过新池第一条。
3. 未命中任何轮次时不切换代理。
