...
test('message router re-syncs contribution mode before AUTO_RUN when sidepanel payload marks accountContributionEnabled=true', async () => {
  ...
  assert.deepStrictEqual(calls, [
    ...
    { type: 'setState', updates: { autoRunSkipFailures: true, autoRunRetryPaypalCallback: false, phoneVerificationCodePrefetchEnabled: false, registrationOnlyModeEnabled: false } },
    { type: 'startAutoRunLoop', totalRuns: 2, options: { autoRunSkipFailures: true, autoRunRetryPaypalCallback: false, phoneVerificationCodePrefetchEnabled: false, registrationOnlyModeEnabled: false, mode: 'restart' } },
  ]);
});
